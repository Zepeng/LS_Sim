...
G4bool LSCherenkov::IsApplicable(const G4ParticleDefinition& aParticleType)
{
...
void LSCherenkov::SetTrackSecondariesFirst(const G4bool state)
{
  fTrackSecondariesFirst = state;
}

void LSCherenkov::SetMaxBetaChangePerStep(const G4double value)
{
  fMaxBetaChange = value*CLHEP::perCent;
}

void LSCherenkov::SetMaxNumPhotonsPerStep(const G4int NumPhotons)
{
  fMaxPhotons = NumPhotons;
}

void LSCherenkov::BuildPhysicsTable(const G4ParticleDefinition&)
{
  if (!thePhysicsTable) BuildThePhysicsTable();
}

void LSCherenkov::SetScaleFactor(const G4double scale) 
{
    fScale = scale;
}

G4double LSCherenkov::GetScaleFactor() const
{
    return fScale;
}
...
//
G4VParticleChange*
LSCherenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
  ...
  if (verboseLevel>0) {
     G4cout <<"\n Exiting from LSCherenkov::DoIt -- NumberOfSecondaries = "
...
void LSCherenkov::BuildThePhysicsTable()
{
...
G4double LSCherenkov::GetMeanFreePath(const G4Track&,
                                           G4double,
                                           G4ForceCondition*)
{
  return 1.;
...
G4double LSCherenkov::PostStepGetPhysicalInteractionLength(
...
G4double
  LSCherenkov::GetAverageNumberOfPhotons(const G4double charge,
                                        const G4double beta, 
			                ...
			                G4MaterialPropertyVector* Rindex) const
{
  ...
  NumPhotons = fScale * NumPhotons;

  return NumPhotons;		
}

void LSCherenkov::DumpPhysicsTable() const
{
